OC.L10N.register(
    ...
    "Absolute path to the python runnable (e.g. \"/usr/bin/python3\"). Can be obtained by `which python3` command. Used when pre-compiled binaries option is not selected." : "Абсолютен път до изпълнимият интерфейс на python (например „/usr/bin/python3“). Може да се получи чрез командата `which python3`.  Използва се, когато не е избрана опцията за предварително компилирани двоични файлове.",
    "Remote/Encrypted file size limit to process" : "Ограничение на размера на отдалечен/криптиран файл за обработка",
    "Maximum file size (in GBytes) for requesting from php core. Used when file is hosted on remote NC instance or have encrypted flag. Must be less than total available RAM size." : "Максимален размер (в Гигабайти) на файл за заявка от php ядрото. Използва се, когато файловете се хостват на отдалечен екземпляр на NC или имат флаг за криптиране. Трябва да е по-малък от общия размер на наличната RAM памет.",
    ...
    "Framework log level" : "Рамка за ниво на регистрационен файл",
    "Used by apps, that are using this framework" : "Използва се от приложения, които използват тази рамка",
    "Cloud Python API" : "API за облачно приложение на Python",
    "Nextcloud Python API (Framework)" : "API за облачно приложение на Nextcloud Python (Рамка)",
